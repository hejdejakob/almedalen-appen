...
const PROGRAM_URL = 'https://almedalsveckan.info/rg/almedalsveckan/officiellt-program/program-2026';
const API_URL = 'https://almedalsveckan.info/appresource/4.356a004a19b2bf461ebf29b3/12.356a004a19b2bf461ebf29be/items';
const EVENT_BASE_URL = 'https://almedalsveckan.info/rg/almedalsveckan/evenemang-almedalsveckan/2026';
const OUTPUT_PATH = path.join(__dirname, 'public', 'almedalen-2026.json');
const PAGE_SIZE = 30;
...
async function main() {
  ...
  const initialState = await page.evaluate(() => {
    return AppRegistry.getInitialState('12.356a004a19b2bf461ebf29be');
  });
  ...
  for (let start = PAGE_SIZE; start < total; start += PAGE_SIZE) {
    ...
    const batch = await page.evaluate(async ({ url, start }) => {
      const r = await fetch(`${url}?start=${start}&startDate=2026-06-22&endDate=2026-06-26&sortOption=relevance`);
      return r.json();
    }, { url: API_URL, start });
