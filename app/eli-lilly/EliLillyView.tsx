...
import EliLillyMap from './EliLillyMap';
...
export type Seminar = {
  ...
  angle: string; competitor_arranged: boolean; client_own: boolean;
};
export type Person = {
...
const THEME_ORDER = ['obesitas', 'prevention', 'alzheimer', 'diagnos', 'access'];
const THEME_COLOR: Record<string, string> = {
  'obesitas': '#fb531a', 'prevention': '#2e9e5b', 'alzheimer': '#8a1f8a',
  'diagnos': '#2f6fdb', 'access': '#a6791f',
};
const CAT_META: Record<string, { label: string; sub: string; color: string }> = {
  ...
  tjansteman: { label: 'Tjänstemän & beslutsfattare', sub: 'TLV, NT-rådet, Socialstyrelsen m.fl. — förstå och träffa', color: '#2f6fdb' },
  patientledare: { label: 'Patientledare', sub: 'allierade att träffa (HOBS, Hjärnfonden, Alzheimerfonden m.fl.)', color: '#2e9e5b' },
  konkurrent: { label: 'Konkurrenter', sub: 'spaning, inte möten. Var positionerar sig Novo Nordisk m.fl.', color: '#8a1f1f' },
};
const CAT_ORDER = ['politiker', 'tjansteman', 'patientledare', 'konkurrent'];

export default function EliLillyView({ seminars, people }: { seminars: Seminar[]; people: Person[] }) {
    ...
    <div style={{ background: '#f7f5e4', minHeight: '100vh', color: '#111' }}>
      <header style={{ background: '#000', color: '#fff', borderBottom: '4px solid #fb531a', padding: '2.4rem 0' }}>
        <div style={wrap}>
          ...
          <h1 style={{ fontFamily: 'var(--font-formula)', fontSize: 'clamp(2.4rem, 6vw, 4.4rem)', margin: 0, lineHeight: 0.94 }}>Eli Lilly i Almedalen</h1>
          <p style={{ opacity: 0.78, marginTop: 14, maxWidth: 720, lineHeight: 1.5 }}>
            En prioriteringskarta över veckans seminarier inom Eli Lillys två stora sakområden: obesitas (vikt, läkemedel och vem som betalar) och Alzheimer (tidig diagnos, nya behandlingar och demensvård) — plus access- och systemfrågorna runtomkring. Var finns frågorna på agendan, vem äger dem, och var bör Lilly vara?
          </p>
      ...
      <main style={wrap}>
        {mapPoints.length > 0 && (
          <section style={{ padding: '2.2rem 0 0.5rem' }}>
            ...
            <p style={lead}>De prioriterade passen geografiskt i Visby. Klicka på en markör för titel och tid.</p>
            <EliLillyMap points={mapPoints} total={core.length} />
          </section>
        )}
        ...
        <section style={{ padding: '2.2rem 0 1rem' }}>
          ...
          <p style={lead}>De {core.length} passen som ligger närmast Lillys sakområden obesitas och Alzheimer. Rekommenderad åtgärd och vinkel per pass. Dag för dag.</p>
        ...
        <section style={{ padding: '1rem 0 1rem' }}>
          ...
          <p style={lead}>Det bredare fältet kring prevention, folkhälsa, diagnostik och access — där Lillys frågor förhandlas, plus de pass där en konkurrent äger scenen och det är värt en spaning.</p>
        ...
        <section style={{ padding: '1.4rem 0 2rem' }}>
          <h2 style={h2}>Nyckelpersoner i Lillys frågor</h2>
          <p style={lead}>
      ...
      <footer style={{ background: '#000', color: '#999', padding: '2.2rem 0', fontFamily: 'var(--mono)', fontSize: '0.74rem', lineHeight: 1.6 }}>
        <div style={wrap}>
          Källa: Almedalsveckans officiella program 2026. Urval, vinklar och kategorisering: Reform Society för Eli Lilly.
          Tiderna anges i svensk tid och kan ändras. ”Konkurrent arrangerar” = pass där ett konkurrerande läkemedelsföretag (t.ex. Novo Nordisk, Eisai/Biogen) står som arrangör (spaning, inte mötesmål).
        </div>
      </footer>
    </div>
  );
  ...
  return (
    <div style={{ ...card, boxShadow: big ? '4px 4px 0 #000' : '2px 2px 0 #000', borderLeft: `5px solid ${color}` }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', margin: '8px 0 6px' }}>
        ...
        {s.client_own && <span style={{ ...miniTag, background: '#000', color: '#fb531a' }}>Eli Lilly</span>}
