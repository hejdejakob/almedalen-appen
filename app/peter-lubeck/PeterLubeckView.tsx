...
export type PersonSched = { name: string; role: string | null; found: boolean; title: string | null; org: string | null; sessions: Session[] };
export type Panel = {
  ...
  niche: string; fit: number; tier: string; angle: string;
};
...
export default function PeterLubeckView({ people, panels = [] }: { people: PersonSched[]; panels?: Panel[] }) {
  const inProgram = people.filter((p) => p.found && p.sessions.length > 0).length;
  ...
  return (
    <div style={{ background: '#f7f5e4', minHeight: '100vh' }}>
      <header style={{ background: '#000', color: '#fff', borderBottom: '4px solid #fb531a', padding: '2.2rem 0' }}>
        <div style={wrap}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: '0.75rem', letterSpacing: '0.12em', color: '#fb531a', textTransform: 'uppercase', marginBottom: 8 }}>Almedalsveckan 2026 · Kundunderlag · Reform Society</div>
          <h1 style={{ fontFamily: 'var(--font-formula)', fontSize: 'clamp(2.2rem, 6vw, 4rem)', margin: 0, lineHeight: 0.95 }}>Peter Lübeck i Almedalen</h1>
          <p style={{ opacity: 0.78, marginTop: 12, maxWidth: 720, lineHeight: 1.5 }}>
            Vilka beslutsfattare du behöver träffa för att flytta frågan om en nationell strategi för dataspel och ett dataspelsinstitut — och var de faktiskt är, dag för dag, i hela Almedalsveckan (inte bara på SpelAlmedalen). Plus en handfull pass utanför spelbubblan där du bör synas. Tider i svensk tid.
          </p>
          <div style={{ display: 'flex', gap: 32, marginTop: 22, fontFamily: 'var(--mono)' }}>
            <div><div style={{ fontSize: '1.8rem', fontFamily: 'var(--font-formula)' }}>{inProgram}</div><div style={statLbl}>personer i programmet</div></div>
            <div><div style={{ fontSize: '1.8rem', fontFamily: 'var(--font-formula)' }}>{panels.length}</div><div style={statLbl}>rekommenderade pass</div></div>
          </div>
        </div>
      </header>
      ...
      <main style={wrap}>
        {mapPoints.length > 0 && (
          <section style={{ padding: '2rem 0 1rem' }}>
            <h2 style={h2}>Var dina målpersoner är</h2>
            <p style={{ color: '#555', marginBottom: 12, fontSize: '0.9rem', maxWidth: 660 }}>Varje markör är ett pass där en av personerna nedan medverkar. Klicka för titel och tid.</p>
        ...
        <section style={{ padding: '1rem 0 2.5rem' }}>
          <h2 style={h2}>Personer att träffa</h2>
          <p style={{ color: '#555', marginBottom: 16, fontSize: '0.9rem', maxWidth: 660 }}>Kultur- och näringsministerkandidater och de mest dataspels-engagerade ledamöterna tvärs partierna, med hela sitt veckoschema så du kan planera att fånga dem.</p>
          {people.map((p) => (
            <div key={p.name} style={personCard}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
                <h3 style={{ ...h2, fontSize: 'clamp(1.15rem, 2.6vw, 1.5rem)', margin: 0 }}>{p.name}</h3>
                <span style={{ fontFamily: 'var(--mono)', fontSize: '0.8rem', color: '#fb531a' }}>{p.sessions.length > 0 ? `${p.sessions.length} pass` : ''}</span>
              </div>
              {p.role && <div style={{ color: '#fb531a', fontFamily: 'var(--mono)', fontSize: '0.74rem', textTransform: 'uppercase', letterSpacing: '0.03em', marginTop: 4 }}>{p.role}</div>}

              {!p.found ? (
                <div style={note}>Ej i 2026-programmet — sök möte separat (kontakta via parti/departement).</div>
              ) : p.sessions.length === 0 ? (
        ...
        {panels.length > 0 && (
          <section style={{ padding: '0 0 3rem' }}>
            <h2 style={h2}>Rekommenderade pass för Peter</h2>
            <p style={{ color: '#555', marginBottom: 18, fontSize: '0.9rem', maxWidth: 680 }}>Pass <strong>utanför spelbubblan</strong> — kulturpolitik, kreativa näringar, regional utveckling — där strategin/institutet landar hos en bredare publik och där beslutsfattarna är. Plus själva institutsdebatten. (SpelAlmedalen kan du redan; använd det för att koordinera, inte för att övertyga politiken.)</p>
      ...
      <footer style={{ background: '#000', color: '#999', padding: '2rem 0', fontFamily: 'var(--mono)', fontSize: '0.75rem' }}>
        <div style={wrap}>Källa: Almedalsveckans officiella program 2026. Urval och sammanställning: Reform Society för Peter Lübeck / Game Habitat. Tider i svensk tid och kan ändras.</div>
      </footer>
    </div>
  );
}
      ...
      {panels.map((p) => (
        <div key={p.eventId} style={panelCard}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', alignItems: 'baseline' }}>
            ...
            <span style={nicheTag}>{p.niche} · {p.fit}/5</span>
          </div>
          ...
          <div style={{ fontSize: '0.82rem', color: '#555', marginTop: 2 }}>
            Arr: {p.arrangers.slice(0, 2).join(', ') || '—'}{p.arrangers.length > 2 ? ' m.fl.' : ''} · {p.speakerCount} talare
          </div>
